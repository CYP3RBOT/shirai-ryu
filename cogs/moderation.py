...
class Moderation(commands.Cog, name="moderation"):
    ...
    async def warning_add(
        ...
        total = await self.bot.database.add_warn(
            str(user.id), str(context.guild.id), str(context.author.id), reason
        )
        embed = discord.Embed(
    ...
    async def warning_remove(
        ...
        total = await self.bot.database.remove_warn(warn_id, str(user.id), str(context.guild.id))
        embed = discord.Embed(
    ...
    async def warning_list(self, context: Context, user: discord.User) -> None:
        ...
        """
        warnings_list = await self.bot.database.get_warnings(str(user.id), str(context.guild.id))
